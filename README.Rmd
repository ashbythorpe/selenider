...
`html_expect()` accepts both functions (`exists`) and calls (`exists()`), allowing expectations to be defined for multiple elements. Note that the first argument to `html_expect()` can be an element *or* a condition.
