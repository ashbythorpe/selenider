...
Using RSelenium as your backend is more complicated. Installing RSelenium can be complicated,
so for a reliable workflow, we recommend using docker. Unfortunately, RSelenium currently
only supports rather old docker images. We currently use RSelenium version 3.141.0. Note
that some of the following advice also applies if you want to use Selenium and docker
manually.

To use docker in a Github Action yaml file, add the following lines (changing the ports and
browser if wanted/needed):
``` yaml
services:
  selenium:
    image: selenium/standalone-firefox:3.141.0
    ports:
      - 4444:4444
```
