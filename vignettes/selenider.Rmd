...
s("#getting-started") |>
  ...
  elem_expect(has_text("Getting Started"))

s(".row") |>
  find_element("#r-foundation") |>
  elem_expect(is_visible, is_enabled)

s("div[role='navigation']") |>
  find_element(".row") |>
  elem_children() |>
  elem_expect(has_at_least(2))

s("#help-with-r") |>
  elem_expect(
    \(x) substring(elem_text(x), 1, 1) == "H"
  )
```
...
you can omit the first argument to `elem_expect()` (but all conditions must be calls).
