test_that("find_each_element() and find_all_elements() work", {
  ...
  elements <- elem_filter(find_all_elements(ss("div"), "button"), has_text("Submit"))

  print("---------")
  print(unclass(elements))
  ...
  #
  # elements <- elem_filter(find_each_element(ss("div"), "button"), has_text("Submit"))
  #
  # elem_expect(elements, has_length(1))
})
